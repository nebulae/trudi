...
- **Background jobs** — long tools run in the background instead of blocking the turn: `ewf_verify`, plaso, carvers, `clamscan_directory`, `recmd_dir` and directory YARA scans return a `job_id` at once; Volatility, EZ directory runs, event-log parsers, `tsk_fls`, mail/chat/USN parsers return one if they pass ~30 s (a short run returns its normal result). Up to 3 jobs run at once, so start independent long tools together and keep working. Poll `misc.job_status(job_id)` between other work — never sit idle on a running job. The finished result carries the citable `_trudi_call_id`. `reason.pre_report_check` blocks while a job is unfinished. A job is lost if the server restarts: re-run the tool.
...
Run the returned `ngrep_pattern` against evidence (`net.ngrep_search`, `strings.grep`, `yara.scan_strings`) before broad enumeration. The first batch must include a knowns-IOC hunt when knowns exist.
...
| `misc.*` | Windows artifacts + email + macros | evtx_dump, regripper, usn_journal, analyzeMFT, Hindsight, ClamAV, PDF/PE, **pff_export**, **readpst_extract**, **densityscout_scan**, **chainsaw_hunt** (Sigma), **capa_analyze** (caps→ATT&CK), **olevba_scan**/**mraptor_scan**, **device_install_inventory** (complete USB/BadUSB device table from setupapi.dev.log), **chat_db_export** (Skype/WhatsApp sqlite → messages/transfers/participants CSVs, strict read-only), **srum_export** (SRUDB.dat ESE → per-table TSV via esedbexport: network usage / app resource usage / id map), **cs_beacon_config** (Cobalt Strike beacon config — C2/sleep/jitter/watermark/spawnto/UA/pubkey hash — from malfind/vadinfo/memmap dumps or a raw image; pass `memmap_listing=` to resolve runtime-table pointers), **batch_run** |
...
| Analyze | Reason about artifacts — processes, network, persistence, TTPs | `reason.hypothesize` per suspicious artifact | Yes — evidence it still needs is a push to Collect |
...
**Loop anatomy:** DAIR is recursive, not linear. Any phase can discover an evidence gap; when it is material to the case question, push **Collect**, gather it, and pop back (new reasoning over collected artifacts is Analyze). A forensic tool called in Report moves the investigation to Collect automatically, and a not-ready `reason.pre_report_check` does the same; pop back to Report when done. **Triage** is re-entered only when what would be triaged is new — another host/image/capture enters scope, the case question changes (or a separate incident emerges), or the opening premise is refuted — and DAIR must give that `triage_reason`; otherwise the server redirects the push to Collect. A new account or identity on the same host is Analyze/Scan work. Report is not a wording exercise: if `reason.synthesize` or `reason.pre_report_check` reports blockers, go back to evidence work before trying to report again.
...
**Phase stack:** the server owns phase state — DAIR is shown the recorded stack, and the model's echo of `current_phase` never moves it (only push/pop and recorded server transitions do). Passing your `phase_stack` is still useful context; it is authoritative only before the first `dair_assess`. JSON list of `{phase, entry_reason, depth}` (newest last). `stack_action`:
...
- `ez_recmd_hive` on SOFTWARE / SYSTEM / SAM — OS+install / ComputerName+timezone+services / local users+last login
- `vol_symbol_check`; `strings_stat_file` on the memory image; `hash_verify_evidence_hash` on each evidence file (once per file per case)

**Do NOT shell out to `dotnet …RECmd.dll` or `/usr/local/bin/vol`** — `source="claude_code_bash"` entries fail the cold-start gate (`protocol_violation: no_active_dair_batch`) and any finding citing them refuses via `mcp_routing`. Skip `ewf_info`/`mmls`/`fsstat`/`vol_info` (slow, uninformative), and skip `ewf_verify` once `hash_verify_evidence_hash` has recorded the result for that image (it re-reads the whole image for the same answer). Pass combined output as `evidence_available`. For pivot Triage entries use whatever artifacts exist (skip the pre-plan reads if the image isn't mounted). `reason.plan` directives inform Collect ordering — re-call mid-Collect if findings change the picture.
...
**One-call finding submission.** Prefer `misc.submit_finding(description, confidence, input_call_ids, idempotency_key, claim, linked_call_id=...)` for one complete draft. Put the typed `record_finding` fields in `claim`. It runs deterministic prerequisites before independent review, then all final gates and an atomic commit. Reuse the exact same key/request after timeout or a lost response; a revised draft needs a new key. Handle `recorded`, `needs-evidence`, `contradicted`, `invalid`, and `retryable-review-failure` explicitly. Never call it merely to preview: success records the finding. No silent confidence downgrade. Existing evaluate/record calls remain available; new reviews with cited inputs bind the exact description, full typed claim, evidence and `supersedes` revision. Keep those identical when recording. See `docs/investigation-review.md` for selectors and limits.
...
4. **"Not shown" is not "not there".** When a review returns `reason: "rows_not_shown"` (or says the deciding rows were not in the selection / not among the shown spans), the reviewer could not SEE them — it did not find them absent. Run a targeted read that returns exactly those rows (`read.output` with `where=`/`query=` on the event id, string or time), cite it, and resubmit. Remove a fact only when a targeted search over the complete source shows it is not there. Do not retract a supported observation to clear a gate; details raised this way are listed in the report.
5. **CHALLENGED/UNCERTAIN is sticky** (gate `challenge_sticky`, keyed by claim — re-wording does not shed it): CONFIRMED/LIKELY refused until a NEW evidence tool call runs AND a later evaluate returns SUPPORTED. A lower tier cannot resolve a factual contradiction; the refusal lists the discriminators to collect.
6. **Refusals are ledgered** (`finding_refused`); re-recording the same claim key + entities with no new evidence tool call since is refused (`refusal_rewording`).
7. **Reviewer answers carry a `RESULT` JSON block** (verdict/blockers/hypotheses/directives), parsed first; each reason/dair entry records `parse_path`.
...
|---|---|---|
| `source` | manifest source id from the refusal (`terminalservices`, `chat_messenger`, `device_inventory`, …) | `absent_from_evidence`\* (cite the listing/search that shows it absent) \| `present_unparseable`\* (in the evidence, no tool parses it) \| `inapplicable` \| `out_of_scope` |
| `tool` | the MCP tool (`ez.pecmd`) | `tool_unavailable` (not installed / cannot run here) \| `absent_from_evidence` \| `inapplicable` \| `out_of_scope` |
| `challenge` | `"<dair_call_id>:<challenge claim>"` | `verified`\* (confirmed by another tool's run) \| `absent_from_evidence` \| `inapplicable` \| `out_of_scope` |
...
Call `reason.audit_findings` explicitly to surface narrated facts without structured findings. It caches successful audits and reviews new narrations incrementally; changed findings trigger a fresh audit. Repeat if `pending_narrations` is nonzero. `reason.pre_report_check` is deterministic and surfaces persisted audit candidates without invoking a model.

Use `supersedes=<active finding call_id>` only for a revision of the same typed proposition. Stale targets, branching revisions and incompatible proposition fields are rejected. Withdraw a separate unsupported claim with `misc.retract_finding(finding_call_id=..., reason=..., input_call_ids=[...])`; history remains available. Legacy ambiguous revision graphs block readiness and require explicit adjudication.

Reasoning and DAIR request one versioned `RESULT` object. Known legacy formats remain readable, but malformed responses get at most one format repair and then fail explicitly. A failed assessment never advances the DAIR phase.
